...
std::string QMKRGBMatrixController::GetQMKVersion()
{
    ...
    hid_read_timeout(dev, (unsigned char*)usb_buf, 65, 10);
...
std::string QMKRGBMatrixController::GetDeviceName()
{
    ...
    hid_write(dev, (unsigned char*)usb_buf, 65);
    hid_read_timeout(dev, (unsigned char*)usb_buf, 64, 1000);
...
unsigned int QMKRGBMatrixController::GetZonesCount()
{
    ...
    /*-----------------------------------------------------*\
    | Repeat transaction until correct response received    |
    \*-----------------------------------------------------*/
    do
    {
        hid_write(dev, (unsigned char*)usb_buf, 65);
...
RGBColor QMKRGBMatrixController::GetLEDColor(unsigned int led)
{
    ...
    while(usb_buf[0x00] != QMK_RGBMATRIX_GET_LED_COLOR);
...
unsigned int QMKRGBMatrixController::GetHSV()
{
    ...
    hid_write(dev, (unsigned char*)usb_buf, 65);
    hid_read_timeout(dev, (unsigned char*)usb_buf, 65, 100);
