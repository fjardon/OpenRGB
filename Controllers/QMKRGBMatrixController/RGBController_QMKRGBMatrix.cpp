...
{
    unsigned int zone_count = qmk_rgb_matrix->GetZonesCount();
    ...
    for(unsigned int i = 0; i < zone_count; i++)
    {
        ...
        if(keyboard_zone.type == ZONE_TYPE_MATRIX)
        {
            ...
            printf("New matrix size rows: %d cols: %d\r\n", led_matrix_rows, led_matrix_columns);
            ...
            for(unsigned int x = 0; x < led_matrix_rows; x++)
            {
                for(unsigned int y = 0; y < led_matrix_columns; y++)
                {
                    ...
                    {
                        matrix_map[led_matrix_columns* x  + y] = 0xFFFFFFFF;
                    }

                    printf("At %d, %d - matrix value %d, name %s\r\n", x, y, led_value, led_name.c_str());
                }
            }
